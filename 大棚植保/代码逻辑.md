...
25. 遍历图像 `src` 的每一行：
	...
	3. 遍历每行的像素数据，每次跳过三个像素（对应 BGR 三个颜色通道），其中 `j` 控制输入图像中的像素位置（初始为 0 ，每次加3），`z` 控制输出图像中的像素位置（初始为 0 ，每次加1）；
...
31. 使用了 OpenCV 中的 `morphologyEx()` 函数对图像进行开运算 (MORPH_OPEN) ，对滤波后的图像进行进一步的形态学处理，处理结果仍覆盖 threshold_img1；（注：开运算（Opening Operation）是形态学处理中的一种基本操作，它是先对图像进行腐蚀操作，再进行膨胀操作。开运算可以用来消除小的噪点，断开物体之间的连接，并且可以平滑物体的边界）；

32. 定义一个整数变量 `line_num`，并将其赋值为 `2`；

33.  `cout` ： `"起始行数检测为："` + line_num 的值，然后换行；

34.  `cout` ： `"otsu计算的分割阈值为："` + otsu_num 的值，然后换行；

35. 声明了四个 `Point2f` 类型的变量，分别是 `p_leftup1`、`p_leftdown1`、`p_rightup1`、`p_rightdown1`。这些变量是 OpenCV 库中用来表示平面上的点坐标，类型为 `Point2f`，表示二维点；

36. 声明了四个 `double` 类型的变量，分别是 `sum_p11`、`sum_p12`、`sum_p13`、`sum_p14`，并初始化它们的值都为 `0`；

37. 声明一个整数变量 `iii`，并将其初始化为 `0`；

38. 声明一个整数变量 `mean_filter_array_size`，并将其初始化为 `5`，表示：导航线滑动均值滤波数组大小；

39. 声明一个整数变量 `time_num`，并将其初始化为 `0`；

40. 声明一个 double 变量 `fps_sum`，并将其初始化为 `0`；

==**节点未关闭：**==

1. 创建一个 geometry_msgs::Twist 类型变量：vel_msg；

2. 创建 double 类型变量：sum_p1、sum_p2、sum_p3 和 sum_p4，并初始化为 0；

3. 创建 int 类型变量：complement_point_num0、complement_point_num1，并初始化为 0；

4. 创建 int 类型变量：point_num0、point_num1，并初始化为 0；

5. 创建 int 类型变量：find_point，并初始化为 0；

6. 从 VideoCapture 类视频流 (`capture`) 中读取帧存入 Mat 类对象 frame ，并将 frame 写入输出视频 (`outputVideo`)；

7. 调用了名为 `BigImage` 的函数，并将当前帧 `frame` 作为参数传递给该函数，函数返回的处理后的图像帧会被赋值给 `frame`，覆盖原始的帧数据。
	...
	6. 使用 `Rect` 类创建一个矩形区域并赋值给  `imgROI`：其左上角坐标为 `(camera_x, camera_y)`，宽度为 `camera_h`，高度为 `camera_w`；
		...
		2. 将提取的感兴趣区域赋值给 `imgROI`；
	7. 使用 `Range` 类构造函数从图像 `img_temp` 中提取指定范围的图像部分，并赋值给 tranPart。
		1. 这里提取的范围是从行0到 479（高度为480），列0到639（宽度为640）的图像数据，并赋值给 `tranPart`。
	8. 使用 `resize` 函数（调整图像大小的函数）将 `imgROI` 调整为与原图像 `img_temp` 相同大小的图像，即宽度为 `img_temp.cols`，高度为 `img_temp.rows`；调整后的图像存储在 `roiResize` 中；
	9. 将调整大小后的 `roiResize` 图像数据复制到 `tranPart` 对应的位置，实现将调整后的感兴趣区域覆盖回原图像 `img_temp` 的指定范围内；
	10. 返回经过处理后的图像副本 `img_temp`，包含了从输入图像 `img` 中提取的感兴趣区域经过调整后的图像数据和覆盖的部分。

8. 将名为 `frame` 的图像赋值给了名为 `src` 的 Mat 类型对象；

9. 创建了一个和 Mat 类对象 `src` 大小相同、单通道（灰度图）的空白图像 `src_gray`，用于存储处理后的图像数据创建了一个和 Mat 类对象 `src` 大小相同、单通道（灰度图，`CV_8UC1`）的空白图像 `src_gray1`，用于存储处理后的图像数据；

10. 使用了 `getTickCount()` 函数来获取当前系统时钟周期数，并将其转换为 `double` 类型，然后将结果赋值给 `double` 型对象 `start` ；

11. 将图像 `src` 的行数赋值给变量 `nl`，即图像的高度；

12. 将图像 `src` 的列数乘以 `3` 赋值给变量 `nc`，即图像的宽度；因为图像通常是多通道的（例如彩色图像通常有三个通道：BGR），所以每个像素占据了三个字节的空间；

13. 图像处理循环，用于将彩色图像 `src` 转换为灰度图像 `src_gray`，并进行一些像素级的颜色处理：
	...
	1. 遍历图像 `src` 的每一行：
		...
		3. 遍历每行的像素数据，每次跳过三个像素（对应 BGR 三个颜色通道），其中 `j` 控制输入图像中的像素位置（初始为 0 ，每次加3），`z` 控制输出图像中的像素位置（初始为 0 ，每次加1）；
			1. 判断当前像素的绿色分量值是否小于蓝色分量值或红色分量值：
				1. 如果条件成立，将输出图像 `src_gray1` 中对应位置的像素值设为 `0`；
				2. 否则，根据公式 `(3 * inData[j + 1] - inData[j] - inData[j + 2])/2` 计算新的像素值，使用 `saturate_cast<uchar>` 进行饱和转换，确保计算结果在 `uchar` 类型的取值范围内（0 到 255），然后将结果存储到 `src_gray1` 中对应位置。

14. 使用 OpenCV 的 `imshow()` 函数显示名为 “灰度图” 的窗口，并在窗口中展示灰度图像 `src_gray`；

15. 整数变量 `time_num` 自加1；

16. 输出：`"timenum"` + time_num的值；

17. 当计数值 time_num 等于 30时：

	1. 调用自定义函数 OTSU，并传入参数：`src_gray`，返回的结果赋值给 otsu_num：
		1. 计算图像的最佳阈值，使得将图像转换为二值图像（黑白图像）时，前景（目标）与背景的类间方差最大化，从而实现自适应的图像分割；
		2. 返回计算得到的最佳阈值；
	2. 输出：`更新otsu计算的分割阈值为：` + otsu_num的值;

18. 使用 OpenCV 库中的 `threshold` 函数对灰度图像 `src_gray` 进行阈值处理（灰度值大于阈值 `otsu_num` 的像素将被设置为 `255`（白色），灰度值小于等于阈值 `otsu_num` 的像素将被设置为 `0`（黑色）），并将结果保存到 `threshold_img` 中；

19. 使用 $3*3$ 的核进行中值滤波（medianBlur），去除图像 `threshold_img` 中的噪声或细节，平滑图像，将结果覆盖图像 `threshold_img`；

20. 创建指定形状（矩形）和大小（$3*3$）的形态学操作的结构元素 kernel（卷积核）；

21. 创建指定形状（矩形）和大小（$3*3$）的形态学操作的结构元素 kernel8（卷积核）；

22. 使用 OpenCV 中的 `erode()` 函数对图像 `threshold_img` 进行腐蚀（erosion）操作；

23. 使用了 OpenCV 中的 `morphologyEx()` 函数对图像进行开运算 (MORPH_OPEN) ，对滤波后的图像进行进一步的形态学处理，处理结果仍覆盖 threshold_img；（注：开运算（Opening Operation）是形态学处理中的一种基本操作，它是先对图像进行腐蚀操作，再进行膨胀操作。开运算可以用来消除小的噪点，断开物体之间的连接，并且可以平滑物体的边界）；

24. 输出：`step1`；

25. 创建一个二维向量：`contours1`，用于存储图像中检测到的轮廓信息；类型是 `Point`，表示二维平面上的一个点，具体来说是由两个整数坐标 `x` 和 `y` 组成的点，用于描述轮廓的连续边界点序列；

26. 创建一个一维向量：`hierarcy1`，通常用于轮廓检测和分析中，用于存储轮廓的层级结构信息；类型是 `Vec4i`，每个 `Vec4i`

	 元素代表一个轮廓的层级关系，通常包含四个整数值，用于描述轮廓之间的父子关系及兄弟关系：

	- `hierarcy1[i][0]`：父轮廓的索引，如果没有父轮廓，则为 `-1`；
	- `hierarcy1[i][1]`：子轮廓的索引，如果没有子轮廓，则为 `-1`；
	- `hierarcy1[i][2]`：前一个兄弟轮廓的索引，如果没有前一个兄弟轮廓，则为 `-1`；
	- `hierarcy1[i][3]`：后一个兄弟轮廓的索引，如果没有后一个兄弟轮廓，则为 `-1`。

27. `abd` 是一个 `vector`，用于存储 `Mat` 类型的对象；

28. `line0`, `line1`, `line2` 分别是 `vector`，用于存储 `Point2f` 类型的对象
